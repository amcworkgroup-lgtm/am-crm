...
app.get('/api/finance', auth, (req, res) => {
  ...
  const revenue = paidPayments.reduce((s,r)=>s+(Number(r.price)||0),0);
  const cost = paidPayments.reduce((s,r)=>s+(Number(r.cost)||0),0);
  const debt = payments.filter(r => r.pay_status !== 'Оплачено').reduce((s,r)=>s+(Number(r.price)||0),0);
  ...
  const methodTotals = {};
  cashRows.forEach(c => {
    const key = c.method || 'Інше';
    methodTotals[key] = (methodTotals[key] || 0) + (c.type === 'in' ? Number(c.amount)||0 : -(Number(c.amount)||0));
  });

  const dayMap = {};
  payments.forEach(r => {
    const d = r.date_in || '—';
    if (!dayMap[d]) dayMap[d] = { date:d, revenue:0, cost:0, expenses:0, profit:0, count:0 };
    dayMap[d].count += 1;
    if (r.pay_status === 'Оплачено') {
      dayMap[d].revenue += Number(r.price)||0;
      dayMap[d].cost += Number(r.cost)||0;
    }
  });
  expenses.forEach(e => {
    const d = e.date || '—';
  ...
  const daily = Object.values(dayMap).sort((a,b)=>a.date.localeCompare(b.date)).slice(-31);

  const masterMap = {};
  payments.forEach(r => {
    const name = r.master || 'Без майстра';
    if (!masterMap[name]) masterMap[name] = { master:name, count:0, revenue:0, cost:0, profit:0 };
    masterMap[name].count += 1;
    if (r.pay_status === 'Оплачено') {
      masterMap[name].revenue += Number(r.price)||0;
      masterMap[name].cost += Number(r.cost)||0;
    }
  });
  ...
  res.json({
    kpi: { revenue, cost, expenses: expenseTotal, profit: revenue - cost - expenseTotal, debt, avg, count: payments.length, paidCount: paidPayments.length },
    payments, expenses, cashRows, daily, byMaster, methodTotals
  });
});
